 ...
 * @since 1.0.0
 */
class Extended_Course_Filter extends Course_Filter {
	/**
	 * Course Category
	 *
	 * @var string
	...
	private function render_terms_hierarchically( $terms, $taxonomy ) {
		...
		$onsit_array = array();	
		$paid_free_cat_array = array();	
		foreach ( $terms as $term ) {

			if($term->slug == 'new' || $term->slug == 'most-popular' || $term->slug == 'trending' || $term->slug == 'featured-course') {
				continue;
			}
			...
			if($term->slug == 'paid-course' || $term->slug == 'free-course') {
				$paid_free_cat_array[] = $term;
				continue;
			}
		...
		// 		echo "<pre>";
		// print_r($onsit_array);echo "</pre>";
		if (!empty($onsit_array)) { ?>
			<div class="mt-5">
			<h3 class="tutor-widget-title">Course Type</h3>
			<?php
				foreach ( $onsit_array as $onsit_term ) { ?>
					<li class="tutor-list-item">
						<label>
							<input type="checkbox" class="tutor-form-check-input"  name="tutor-course-filter-<?php echo esc_attr( $taxonomy ); ?>" value="<?php echo esc_attr( $onsit_term->term_id ); ?>" <?php echo esc_attr( $onsit_term->term_id == $term_id ? 'checked="checked"' : '' ); ?>/>
							<?php echo esc_html( $onsit_term->name ); ?>
						</label>
					</li>
				<?php 
				}
		...
		if (!empty($paid_free_cat_array)) { ?>
			<div class="mt-5">
			<h3 class="tutor-widget-title">Price</h3>
			<?php
				foreach ( $paid_free_cat_array as $paid_free_cat_term ) { ?>
					<li class="tutor-list-item">
						<label>
							<input type="checkbox" class="tutor-form-check-input"  name="tutor-course-filter-<?php echo esc_attr( $taxonomy ); ?>" value="<?php echo esc_attr( $paid_free_cat_term->term_id ); ?>" <?php echo esc_attr( $paid_free_cat_term->term_id == $term_id ? 'checked="checked"' : '' ); ?>/>
							<?php echo esc_html( $paid_free_cat_term->name ); ?>
						</label>
					</li>
				<?php 
				}
