...
add_filter(
	...
	function( $defaults ) {
		if ( isset( $defaults['class'] ) ) {
			$defaults['class'] = 'edd-add-to-cart button white edd-submit edd-has-js btn ';
		}
		return $defaults;
	}
);
...
<?php
	...
	$enroll_btn = '<div class="list-item-button  d-flex justify-content-center">' . apply_filters( 'tutor_course_restrict_new_entry', '<a href="' . get_the_permalink() . '" class="btn  w-100">' . __( 'Enroll Course', 'tutor' ) . '</a>' ) . '</div>';
...
if ( tutor_utils()->is_course_purchasable() ) {
	...
	if ( 0 == $maximum_students ) {
		?>
				<div class="list-item-button  d-flex justify-content-center">
			<?php
